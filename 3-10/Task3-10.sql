...
SELECT GoodsCode, SUM(Quantity) AS TotalQuantity FROM Stocks GROUP BY GoodsCode;
...
FROM Shop s JOIN Area a ON s.AreaCode = a.AreaCode;

-- 8. 「新宿」店の在庫数が10以上の商品の商品コード（GoodsCode）、商品名（GoodsName）、在庫数（quantity）を抽出して下さい。
SELECT st.GoodsCode, g.GoodsName, st.Quantity
FROM Stocks st JOIN Goods g ON st.GoodsCode = g.GoodsCode
WHERE st.ShopCode = '001'
